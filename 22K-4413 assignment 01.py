...
def preprocess(text, stopwords):
    ...
    tokens = [token.lower() for token in tokens]  # converting tokens to lowercase for uniformity
    tokens = [token for token in tokens if token.isalnum() and token not in stopwords]  # remove stopwords and non-alphanumeric tokens
...
# this function loads abstracts from a zip file using multiple encodings some files are encoded in different character sets, so multiple encodings are tried ['latin-1', 'ISO-8859-1', 'Windows-1252'] are common encodings for text files
...
def initializeApp():
    ...
    stopwordsFile = "Stopword-List.txt"
